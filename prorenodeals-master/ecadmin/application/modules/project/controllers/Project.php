		...
		if($bid_count > 0){
			...
			if($this->input->post('submit')){
				...
				if($this->form_validation->run()){
					...
					$insert = $this->db->insert('project_description', $ins);
					...
					if(count($bidders) > 0){
						foreach($bidders as $bidder){
							...
							$notification = "Admin edited description for project ".$project_title;
							$link = "jobdetails/details/".$project_id;
							$this->notification_model->log($employer_id, $bidder_id, $notification, $link);
						}
			...
			$this->layout->view('edit_description', $lay, $data);
		}else{
