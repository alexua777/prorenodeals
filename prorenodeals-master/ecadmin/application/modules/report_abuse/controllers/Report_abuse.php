...
class Report_abuse extends MX_Controller {
    ...
    public function index() {
		redirect(base_url('report_abuse/list_all'));
	}
	...
	public function list_all_project($limit_from=0){
		...
		$data['all_data'] = $this->report_abuse_model->get_project_list($srch,  $start, $per_page);
		$data['all_data_count'] = $this->report_abuse_model->get_project_list($srch, '', '', FALSE);
		
        $config = array();
        $config["base_url"] = base_url()."report_abuse/list_all_project";
	...
	public function list_all_user($limit_from=0){
		...
		$data['all_data'] = $this->report_abuse_model->get_user_list($srch,  $start, $per_page);
		$data['all_data_count'] = $this->report_abuse_model->get_user_list($srch, '', '', FALSE);
		
        $config = array();
        $config["base_url"] = base_url()."report_abuse/list_all_user";
        $config["total_rows"] = $data['all_data_count'];
        $config["per_page"] = $per_page;
		$config["uri_segment"] = 3;
        $config['use_page_numbers'] = TRUE;
