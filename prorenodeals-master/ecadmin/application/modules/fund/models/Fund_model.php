...
*/
	...
	public function getWalletTxn($srch=array(), $limit=0, $offset=30, $for_list=TRUE){
		...
		if(!empty($srch['year'])){
			$this->db->where("YEAR(tr.datetime) = '{$srch['year']}'");
		}
	...
	public function getTxnHistoryNew($srch=array(), $limit=0, $offset=30, $for_list=TRUE){
		...
		$this->db->select("tn.*,c.title_tkey as `key`,c.description_tkey as description")
				->from('transaction_new tn')
				->join('wallet_transaction_type c', 'c.wallet_transaction_type_id=tn.txn_type', 'LEFT');
	...
	public function getProjectTxnHistoryNew($srch=array(), $limit=0, $offset=30, $for_list=TRUE){
		...
		$this->db->select("p.title,tn.*,c.title_tkey as `key`,c.description_tkey as description")
	...
	public function getDisputeMessages($srch=array(), $limit=0, $offset=30, $for_list=TRUE){
		...
		$this->db->where('dm.dispute_id', $srch['dispute_id']);
	...
	public function getDisputeHistory($dispute_id=''){
		...
		$this->db->where('dh.dispute_id', $dispute_id);
