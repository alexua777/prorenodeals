						...
						<thead>  	
						 ...
						 <tbody>
							<tr>
								<td>
									<?php
									$escrow_detail = get_row(array(
										'select' => '*',
										'from' => 'escrow_new',
										'where' => array('escrow_id' =>  $dispute_row['escrow_id']),
									));
									$title = '';
								...
								<td>
									$<?php echo !empty($escrow_detail['amount']) ? $escrow_detail['amount'] : '0.00'; ?>
								</td>
								<td>
									<?php if(!empty($dispute_row) && ($dispute_row['dispute_settled'] == '0')){ ?>
									...
									<form onsubmit="handleDispute(this, event)" id="close_dispute_form" style="display: none; ">
										<input type="hidden" name="dispute_id" value="<?php echo $dispute_row['dispute_id']; ?>"/>
										<?php
										...
										$milestone_amount = $escrow_detail['amount'];
										/* $commission = (($milestone_amount * SITE_COMMISSION) / (100 + SITE_COMMISSION)); */
										$commission =$escrow_detail['admin_fee'];
										$amount_to_distribute = $escrow_detail['bidder_amt'];
										/* get_print( $dispute_row); */
										...
										Tax (-) : <b>$<?php echo $escrow_detail['tax_amount']; ?></b><br/>
