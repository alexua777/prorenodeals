  ...
  public function message($did)
	...
	public function status_txn_new(){
		...
		if($this->input->post()){
			...
			$txn_type = getField('txn_type', 'transaction_new', 'txn_id', $txn_id);
			...
			if($cmd == 'approve'){
				...
				if($txn_row){
					foreach($txn_row as $k => $v){
						...
						$fname = getField('fname', 'user', 'user_id', $wallet_user);
							
						if($txn_type == WITHDRAW_WALLET_FUND){
							
							$withdraw_amount = $v['debit'];
							if($cmd && $wallet_user){
								...
								send_layout_mail('withdraw_payment_request_for_freelancer', $mail_param, $f_email);
							}
						}else{
							$mail_param = array(
								'ACTION' => 'rejected',
	...
	public function view_diputes($dispute_id='', $limit_from=0){
		$page = ($limit_from) ? $limit_from : 0;
        $per_page = 60;
        $start = 0;
        if ($page > 0)
		...
		$data['all_dispute_history'] = $this->fund_model->getDisputeHistory($dispute_id);
		
		$data['dispute_row'] = $this->db->where(array('dispute_id' => $dispute_id))->get('dispute')->row_array();
		
        $config = array();
        $config["base_url"] = base_url()."fund/view_diputes/".$dispute_id;
	...
	public function close_dispute(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if(!empty($escrow_check)){
				...
				$amount_dispute = $dispute_history_row['employer_amount'];
				$worker_amount  = $dispute_history_row['worker_amount'];
				...
				$employer_info = get_row(array('select' => 'user_id,fname,lname,email','from' => 'user', 'where' => array('user_id' => $dispute_history_row['employer_id'])));
				...
				$receiver_info = array(
					'name' => $employer_info['fname'].' '.$employer_info['lname'],
					'address' => getUserAddress($employer_info['user_id']),
				);
				
				$invoice_data = array(
					'sender_id' => $user_info['user_id'],
					'receiver_id' => $employer_info['user_id'],
					'invoice_type' => 1,
					'sender_information' => json_encode($sender_info),
					'receiver_information' => json_encode($receiver_info),
					'receiver_email' => $employer_info['email'],
				
				);
				
				$new_inv_id = create_invoice($invoice_data); // creating invoice
				
				$invoice_row_data[] = array(
					'invoice_id' => $new_inv_id,
					'description' => 'Dispute settlement #'.$project_id,
					'per_amount' => ($escrow_check['bidder_amt']+$escrow_check['admin_fee']),
					'unit' => '-',
					'quantity' => 1,
				);
				$invoice_row_data[] = array(
				...
				$invoice_row_data[] = array(
					'invoice_id' => $new_inv_id,
					'description' => 'HST',
					'per_amount' => ($escrow_check['tax_amount']),
					'unit' => '-',
					'quantity' => 1,
				);
				...
				add_project_invoice($project_id, $new_inv_id);
					...
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $employer_wallet_id, 'credit' => $dispute_history_row['employer_amount'], 'ref' => $milestone_id, 'info' => 'Dispute settlement received #'.$project_id));
					
					
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $bidder_wallet_id, 'credit' => ($dispute_history_row['worker_amount'] + $commission), 'ref' => $milestone_id, 'info' => 'Dispute settlement received #'.$project_id));
					
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => TAX_WALLET, 'credit' => $escrow_check['tax_amount'] , 'ref' => $milestone_id , 'info' => 'HST Charge #'.$project_id));
					...
					/* $new_txn_id_3 = $this->transaction_model->add_transaction(TAX_PAYMENT,   $dispute_history_row['worker_id']);
					
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id_3, 'wallet_id' => $bidder_wallet_id, 'debit' => $escrow_check['tax_amount'], 'ref' => $bid_id , 'info' => 'HST Charge #'.$project_id));
					
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id_3, 'wallet_id' => TAX_WALLET, 'credit' => $escrow_check['tax_amount'] , 'ref' => $bid_id , 'info' => 'HST Charge #'.$project_id)); */
				...
				$this->db->where('dispute_id', $dispute_id)->update('dispute', array('dispute_settled' => 1));
				...
				$this->db->where('invoice_id', $new_inv_id)->update('invoice_main', array('is_paid' => date('Y-m-d H:i:s'))); 
				$this->db->where('invoice_id', $inv_id)->update('invoice_main', array('is_paid' => date('Y-m-d H:i:s'))); // commission invoice mark as paid
				
				$this->db->where('dispute_id', $dispute_id)->update('dispute_time_track', array('dispute_status' => 'S'));
				...
				if($p_type == 'F'){
					...
					$notification = "Disputed milestone has settled by admin";
					
					$link = 'projectdashboard/dispute_room/'.$dispute_id;
					...
					$SETTLEMENT_AMOUNT = ($dispute_history_row['employer_amount']+$dispute_history_row['worker_amount']);
					$PROJECT_TITLE = getField('title', 'projects', 'project_id', $dispute_history_row['project_id']);
					$e_name = getField('fname', 'user', 'user_id', $dispute_history_row['employer_id']);
					$e_email = getField('email', 'user', 'user_id', $dispute_history_row['employer_id']);
					$data_param = array(
						'USER' => $e_name,
						'PROJECT_TITLE' => $PROJECT_TITLE,
						'SETTLEMENT_AMOUNT' => $SETTLEMENT_AMOUNT,
						'RECEIVED_AMOUNT' => format_money($dispute_history_row['employer_amount'], TRUE),
					);
					send_layout_mail('admin_dispute_settlement', $data_param, $e_email);
					
					$f_name = getField('fname', 'user', 'user_id', $dispute_history_row['worker_id']);
					$f_email = getField('email', 'user', 'user_id', $dispute_history_row['worker_id']);
					$data_param = array(
						'USER' => $f_name,
						'PROJECT_TITLE' => $PROJECT_TITLE,
						'SETTLEMENT_AMOUNT' => $SETTLEMENT_AMOUNT,
						'RECEIVED_AMOUNT' => format_money($dispute_history_row['worker_amount'], TRUE),
					);
					send_layout_mail('admin_dispute_settlement', $data_param, $f_email);
	...
	public function tax_wallet($limit_from=0){				$page = ($limit_from) ? $limit_from : 0;        $per_page = 20;        $start = 0;        if ($page > 0)        {            for ($i = 1; $i < $page; $i++)            {                $start = $start + $per_page;            }        }		$data['srch'] = $srch = $this->input->get();		$data['data'] = $this->auto_model->leftPannel();		$lay['lft'] = "inc/section_left";		$wallet_id = TAX_WALLET;		$data['wallet_id'] = $srch['wallet_id'] = $wallet_id; 				$data['all_data'] = $this->fund_model->getWalletTxn($srch,  $start, $per_page);		$data['all_data_count'] = $this->fund_model->getWalletTxn($srch, '', '', FALSE);				$data['debit_total'] = $this->fund_model->wallet_debit_balance($wallet_id);		$data['credit_total'] = $this->fund_model->wallet_credit_balance($wallet_id);		        $config = array();        $config["base_url"] = base_url()."fund/tax_wallet/";        $config["total_rows"] = $data['all_data_count'];        $config["per_page"] = $per_page;		$config["uri_segment"] = 3;        $config['use_page_numbers'] = TRUE;		        $this->pagination->initialize($config);                $config["page"]  =	$config["per_page"];		$config['full_tag_open'] = '<nav aria-label="Page navigation example"><ul class="pagination">';		$config['full_tag_close'] = '</ul></nav>';		$config['first_link'] = 'First';		$config['first_tag_open'] = '<li class="page-item">';		$config['first_tag_close'] = '</li>';		$config['num_tag_open'] = '<li class="page-item">';		$config['num_tag_close'] = '</li>';		$config['cur_tag_open'] = '<li class="page-item active"><a href="javascript:void(0)" class="page-link">';		$config['cur_tag_close'] = '</a></li>';		$config['last_link'] = 'Last';		$config['last_tag_open'] = '<li class="page-item last">';		$config['last_tag_close'] = '</li>';		$config['next_link'] = 'Next'.' &gt;&gt;';		$config['next_tag_open'] = '<li class="page-item xyz">';		$config['next_tag_close'] = '</li>';		$config['prev_link'] = '&lt;&lt;'.'Previous';		$config['prev_tag_open'] = '<li class="page-item">';		$config['prev_tag_close'] = '</li>'; 		$this->pagination->initialize($config);        $data["links"] = 	$this->pagination->create_links();				$data['wallet_type'] = 'tax';		$this->layout->view('wallet_detail', $lay, $data); 	}		public function txn_to_csv($wallet_id=''){		$data['srch'] = $srch = $this->input->get();		$data['wallet_id'] = $srch['wallet_id'] = $wallet_id; 		$data['all_data'] = $this->fund_model->getWalletTxn($srch,  0, 1000);		$debit_total = $this->fund_model->wallet_debit_balance($wallet_id);		$credit_total = $this->fund_model->wallet_credit_balance($wallet_id);		$csv_data = array();				$csv_data[]  = array('Txn ID', 'Date', 'Info', 'Status', 'Debit', 'Credit');				if($data['all_data']){			foreach($data['all_data'] as $key => $val){				$status = '';				switch($val['status']){					case 'Y' : 						$status = 'Success';					break;					case 'P' : 						$status = 'Pending';					break;					case 'N' : 						$status = 'Rejected';					break;				}								$csv_data[] = array($val['txn_id'], $val['datetime'], $val['info'], $status, $val['debit'], $val['credit']);							}						/* $csv_data[] = array('', '', '', '', 'Total :'.$debit_total, 'Total :'.$credit_total); */		}		$this->load->helper('csv');		array_to_csv($csv_data, 'Transaction-'.date('Y-m-d').'.csv');			}
