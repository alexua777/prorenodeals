...
public function send_msg(){
	...
	if($this->input->post()){
		...
		if($insert){
			$from=ADMIN_EMAIL;
			$last_seen=$this->auto_model->getFeild('last_seen','user','user_id',$this->input->post("recipient_id"));     
			...
			if(!$status){
				...
				$sender_name = getField('fname', 'user', 'user_id', $this->input->post("sender_id"));
				$template='pm_alert';
				$data_parse=array('name'=>$fname." ".$lname, 'username' => $sender_name);
...
public function get_left_msg($type=''){
	...
	if(count($message) > 0){
		...
		foreach($message as $k => $v){
			...
			echo '<li class="messagtext2" id="msg_project_'.$v['project_id'].'_u_'.$v['sender_user'].'">';
			if($v['unread_msg']>0){
			echo '<div class="msg-count"><span class="notification" id="unread">'.$v['unread_msg'].'</span></div>';
			}
			echo '<div class="setting-circle dropdown hidden"><i class="fa fa-ellipsis-h"></i></div><a id="anchor" href="javascript:void(0);" onclick="rampage(\''.$v['project_id'].'\',\''.$v['sender_user'].'\' , $(this));"><div class="message-avatar"><i class="icon-feather-message-square"></i></div><div class="message-by"><div class="message-by-headline"> <h5>'.$sender_fname.' , '.$recipient_fname .'</h5></div><p>'.$project_name.'</p></div></a><p style="font-weight:normal;padding:5px 0px; display:none;">'.$msg.'</p>';
			echo '</li>';
		}
