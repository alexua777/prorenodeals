...
class projectdashboard_model extends BaseModel {
	...
	public function getWeeklyMinDeposit($project_id=''){
		$amount = 0;
		$active_freelancers = $this->db->where(array('project_id' => $project_id, 'is_project_start' => 1, 'is_contract_end' => 0))->get('project_schedule')->result_array();
		if($active_freelancers){
			foreach($active_freelancers as $freelancer){
				$data=get_row(array('select'=>'total_amt,available_hr','from'=>'bids', 'where'=>array('project_id'=>$project_id,'bidder_id'=>$freelancer['freelancer_id'])));
				$min_amount = $data['total_amt'] * $data['available_hr'];
				$amount += $min_amount;
			}
		}
	...
	public function checkProjectDeposit($min_deposit=0 , $project_id=''){
		...
		$weekly_min_deposit = $this->getWeeklyMinDeposit($project_id);
		...
		$min_deposit += $weekly_min_deposit;
	...
	public function pay_hourly_invoice($invoice_id='', $project_id=''){
		...
		if(count($tracker_rows) > 0){
			...
			$remaining_bal_str =  CURRENCY.''.number_format($remaining_deposit ,  2);
			...
			if($remaining_deposit < $total_cost_new){
				...
				$html = '<div><b>'.$diff_str.'</b> is needed in your deposit to process this invoice. Current wallet balance <b>'.$wallet_bal.'</b>  and deposit balance is <b>'.$remaining_bal_str.' </b> . Click on the below button to proceed . <div><br/> <a href="'.$link .'" class="btn btn-site">Add fund and process invoice</a> </div> </div>';
				$msg['msg'] = '<div class="info-error">Not enough balance in your deposit</div>'.$html;
				$msg['status']=0;
				$error++;
			}
	...
	public function getDisputes($srch=array(), $limit=0, $offset=30, $for_list=TRUE){
		/* $this->db->select("e.*,p.title as project_title,pm.title,pm.description,pm.send_to_admin")
				...
				->join('bids b', 'b.id=pm.bid_id', 'LEFT')
				->where(array('e.status' => 'D')); */
		$this->db->select("d.*,p.title as project_title,pm.title,pm.description,e.amount,e.bidder_amt,e.admin_fee,e.tax_amount")
				->from('dispute d')
				->join('escrow_new e', 'e.escrow_id=d.escrow_id', 'LEFT')
				->join('projects p', 'd.project_id=p.project_id', 'LEFT')
				->join('project_milestone pm', 'd.milestone_id=pm.id', 'LEFT');
				/* ->join('bids b', 'b.id=pm.bid_id', 'LEFT') */
				/* ->where(array('e.status' => 'D')); */
		
		
		$this->db->where("(d.employer_id = {$srch['user_id']} OR d.worker_id = {$srch['user_id']})");
		
		if($for_list){
			$result = $this->db->limit($offset, $limit)->order_by('d.dispute_id', 'DESC')->get()->result_array();
	...
	public function getAllPayments(){
		...
		$this->db->select('pm.*')
				->from('projects p')
				->join('bids b', 'b.bidder_id=p.bidder_id AND b.project_id=p.project_id')
				->join('project_milestone pm', 'pm.bid_id=b.id');
