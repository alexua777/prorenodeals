...
class Projectdashboard_new extends MX_Controller {
	...
	private function freelancer_overview($project_id=''){
		...
		if(!$this->session->userdata('user')){
			$ref = 'projectroom/contractor/overview/'.$project_id;
	...
	private function freelancer_milestone($project_id=''){
		if(!$this->session->userdata('user')){
			$ref = 'projectroom/contractor/milestone/'.$project_id;
			redirect(base_url('login?refer='.$ref));
		}
		$data = array();
		$user = $this->session->userdata('user');
	...
	private function employer_overview($project_id=''){
		if(!$this->session->userdata('user')){
			$ref = 'projectroom/employer/overview/'.$project_id;
			redirect(base_url('login?refer='.$ref));
		}
		$data = array();
		$weekly_min_deposit = $this->projectdashboard_model->getWeeklyMinDeposit($project_id);
		$data['min_deposit'] = $this->min_deposit + $weekly_min_deposit;
		$user = $this->session->userdata('user');
		$user_id = $user[0]->user_id;
		$data['user_id'] = $user_id ;
		$data['project_id'] =  $project_id;
	...
	private function employer_milestone($project_id=''){
		if(!$this->session->userdata('user')){
			$ref = 'projectroom/employer/milestone/'.$project_id;
			redirect(base_url('login?refer='.$ref));
		}
		$data = array();
		$user = $this->session->userdata('user');
		$user_id = $user[0]->user_id;
		$data['user_id'] = $data['login_user_id'] = $user_id ;
		$data['project_id'] =  $project_id;
			...
			if(!is_employer($user_id , $req_row['project_id'])){
				...
				if($action == 'A'){
					...
					if(!empty($req_row)){
						...
						$prev_count = $this->db->where($p_schedule)->count_all_results('project_schedule');
						
						if($prev_count == 0){
							$p_schedule['manual_request_enable'] = 1;
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				if($amount > $wallet_balance){
					$return_url = urlencode(base_url('projectroom/employer/overview/'.$project_id));
					$remaining_bal = ceil($amount - $wallet_balance);
					...
					$diff = ($amount - $wallet_balance);
					$paypal_comm = ($diff * $paypal_fee_percent)/100;
					$paypal_comm += 0.35;
					
					$amount_to_pay = $diff + $paypal_comm;
					$amount_to_pay_str = number_format($amount_to_pay, 2);
					$paypal_fee_percent_str = str_replace('.00', '', $paypal_fee_percent);
					$add_fund_url = base_url('myfinance/add_fund_paypal?amt='.$amount_to_pay.'&cmd=deposit_project_fund&project_id='.$project_id);
					$html = '<p>Amount : '.CURRENCY.$amount.'</p>';
					$html .= '<p>Paypal fee('.$paypal_fee_percent_str.' + 0.35) : '.CURRENCY. number_format($paypal_comm, 2) .'</p>';
					$html .= '<p>Total : '.CURRENCY. number_format(($amount + $paypal_comm), 2).'</p>';
					$json['errors']['balance'] = $html. '<div class="info-error">You need '.CURRENCY.$amount_to_pay_str.' to deposit. <a href="'.$add_fund_url.'">Click here</a> to add remaining fund </div>';
	...
	public function update_review_employer(){
		...
		if(post() && $this->input->is_ajax_request() && $user_id && $project_id && $employer_id){
			...
			if($error == 0){
				...
				if(check_user_review($review_to, $project_id)){
					$review_where['review_by_user'] = $user_id;
					$review_where['review_to_user'] = $review_to;
					$review_where['project_id'] = $project_id;
					
					$review_where2['review_to_user'] = $user_id;
					$review_where2['review_by_user'] = $review_to;
					$review_where2['project_id'] = $project_id;
	...
	public function milestone_request_ajax(){
		...
		if($this->input->is_ajax_request()){
			...
			$AMOUNT = getField('amount', 'project_milestone', 'id', $milestone_id);
			...
			$data_parse = array(
				'EMPLOYER' => $employer_name,
				'FREELANCER' => $freelancer_username,
				'AMOUNT' => format_money($AMOUNT, TRUE),
			);
			send_layout_mail($template, $data_parse, $to);
	...
	public function resolution_center($limit_from=0){
		$data = array();
		$user = $this->session->userdata('user');
		$user_id = $user[0]->user_id;
		...
		$data['dispute_list'] = $this->projectdashboard_model->getDisputes($srch, $limit, $offset);
		$data['total_records'] = $this->projectdashboard_model->getDisputes($srch, '', '', FALSE);
		
		/*Pagination Start*/
		$config['base_url'] = base_url('projectdashboard_new/resolution_center');
		...
		$breadcrumb=array(
			array(
					'title'=>'Resolution center','path'=>''
			)
		);
		$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,'Resolution center');
		$data['active_tab'] = 'invoices';
		
		$this->layout->view('dispute_list','',$data,'normal');   
	}
	
	public function add_process_invoice(){
		$data = array();
		$user = $this->session->userdata('user');
		$user_id = $user[0]->user_id;
		...
		if(count($tracker_rows) > 0){
			...
			$wallet_bal = get_wallet_balance($employer_wallet_id);
			...
			if($remaining_deposit < $total_cost_new){
				...
				if($wallet_bal >=  $diff){
					$this->myfinance_model->deposit_project_fund($project_id, $user_id, $diff, FALSE);
					$this->projectdashboard_model->pay_hourly_invoice($invoice_id, $project_id);
					$referer = $this->input->server('HTTP_REFERER');
					redirect($referer);
				}else{
					$total_sum = 0;
					$paypal_fee_percent = getField('deposite_by_paypal_commission', 'setting', 'id', 1);
					// add fund through paypal and process invoice 
					$diff = $diff - $wallet_bal;
					$paypal_comm = ($diff * $paypal_fee_percent)/100;
					$paypal_comm += 0.35;
					$total_sum = $diff + $paypal_comm;
					
					$paypal_url = base_url('myfinance/add_fund_paypal?cmd=process_invoice&amt='.$total_sum.'&invoice_id='.$invoice_id.'&project_id='.$project_id);
	...
	public function get_all_payments(){
		$data['set_milestone_list'] = $this->projectdashboard_model->getAllPayments();
		$this->load->view('employer_milestone_ajax', $data);
	}
