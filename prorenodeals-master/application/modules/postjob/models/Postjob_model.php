...
class Postjob_model extends BaseModel {
   ...
   public function post_project() {
	$this->load->helper('date');
	 $this->load->library('MY_Validation');
       ...
       $ball="";
		...
		//$subskill=$subskill;
		$project_type=$this->input->post('project_type');
		$deadline = $this->input->post('bidding_deadline');
	...
	if(!$this->session->userdata('user')){					
		...
		if($postal_code==''){			
			$msg['status']='FAIL';			
			$msg['errors'][$i]['id']='postal_code';			
			$msg['errors'][$i]['message']="Please Enter Postal Code";			
			$i++;					
		}else if(!preg_match("/^[ABCEGHJ-NPRSTVXY]\d[ABCEGHJ-NPRSTV-Z][ -]?\d[ABCEGHJ-NPRSTV-Z]\d$/i", $postal_code)){
			$msg['status']='FAIL';			
			$msg['errors'][$i]['id']='postal_code';			
			$msg['errors'][$i]['message']="Please Enter Valid Postal Code";			
			$i++;		
		}		
		if($deadline==''){
			$msg['status']='FAIL';
			$msg['errors'][$i]['id']='bidding_deadline';
			$msg['errors'][$i]['message']=__('postjob_please_enter_bid_deadline','Please Enter Bidding Deadline');
			$i++;
		}
		if($title==''){
			$msg['status']='FAIL';
	   ...
	   $place_quote=$this->input->post('place_quote');   
		if(!$place_quote){
			$place_quote = 0;
		}
		   ...
		   if(!empty($hr_per_week) && !is_numeric($hr_per_week)){
			...
			$i++;                    
				  ...
				  if($bmax<=$bmin){
					$msg['status']='FAIL';
					$msg['errors'][$i]['id']='budgetmax';
					$msg['errors'][$i]['message']= 'Maximum budget must be greater than minimum budget';
					$i++; 
				  }
				  if($bmin<$min_budget_check) {
					$msg['status']='FAIL';
					$msg['errors'][$i]['id']='budgetmin';
					$msg['errors'][$i]['message']='Minimum budget must be greater or equal to '.$min_budget_check;
					$i++; 
				  }
			  }  
				...
				/* if($featured == 'Y'){
					...
					$data = array(
							   ...
							   /*  'category' => $this->input->post('category_id'),
								...
								'postal_code'=>filter_data($this->input->post('postal_code')),	
								'project_start_on'=>filter_data($this->input->post('project_start_on')),
								'bidding_deadline'=>filter_data($this->input->post('bidding_deadline')),
							);
					...
					if($pid){
						...
						if($this->input->post('featured')=="Y"){ 
							...
							if(($this->input->post('project_type')=="F" && $balance<FIXED_RATE) || ($this->input->post('project_type')=="H" && $balance<HOURLY_RATE)){
								...
								$insufficient_bal = true;
									...
									$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $user_wallet_id, 'debit' => FIXED_RATE, 'ref' => $ref , 'info' => 'Project featured fee #'.$project_id));
									
									$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => PROFIT_WALLET, 'credit' => FIXED_RATE, 'ref' => $ref, 'info' => 'Project featured fee received #'.$project_id));
								...
								$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $user_wallet_id, 'debit' => HOURLY_RATE, 'ref' => $ref , 'info' => 'Project featured fee #'.$project_id));
								
								$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => PROFIT_WALLET, 'credit' => HOURLY_RATE, 'ref' => $ref, 'info' => 'Project featured fee received #'.$project_id));
						...
						if($featured == 'Y'){
							$expiry_date = date("Y-m-d",  strtotime('+30 day', strtotime($expiry_date)));
							$this->db->where('id', $pid)->update('projects', array('expiry_date' => $expiry_date, 'featured' => $featured));
						}
					...
					//	$this->auto_model->send_email($from,$to,$template,$data_parse);   
						...
						if($insufficient_bal){
							$payment_link = base_url('myfinance/add_fund_paypal?cmd=featured_project&project_id='.$project_id);
							$msg['message']="<h3 class='postjob-alert-title'><span style='margin-right:10px'><i class='fa fa-check'></i></span>Job Successfully Posted.</h3>
							<p>You don't have sufficient fund in your wallet . Would you like to proceed to make this project featured ?</p> <ul class='alert-choice'><li><a  href='".$payment_link."'>Yes, Proceed </a></li><li><a href='".VPATH."jobdetails/details/".$project_id."'>No , Preview your job </a></li> <li><a  href='".VPATH."postjob/editjob/".$pid."'> Edit your job </a> </li></ul>";
   ...
   public function updatepost_project() {
	...
	$i=0;
	   ...
	   $place_quote=$this->input->post('place_quote');   
		...
		if($place_quote > 0){
			...
			$i++;                   
				  ...
				  if($bmax<=$bmin){
					$msg['status']='FAIL';
					$msg['errors'][$i]['id']='budget';
					$msg['errors'][$i]['message']="Maximum budget should be greater than minimum budget";
					$i++; 
				  }
				  if($bmin<$min_budget_check) {
					$msg['status']='FAIL';
					$msg['errors'][$i]['id']='budget';
					$msg['errors'][$i]['message']="Minimum Budget should be greater than $min_budget_check";
					$i++; 
				  }
			  }   
               if($multi_freelancer=='Y'){
		...
		$additional_info = filter_data($this->input->post('additional_info'));
		$reopen = filter_data(post('reopen'));
		if(!$reopen){
			$reopen = 0;
		}
       ...
       if($i==0){
		   if(!empty($additional_info)){
			   $this->edit_job_description($additional_info, $project_id);
			   $this->resetBid($project_id);
		   }
			...
			$input_data="";
				...
				if($this->input->post('description') && $bid_count == 0){
					$data['description'] = filter_data($this->input->post('description'));
				}
			
			
			/* if($feature_added){
				$expiry_date = getField('expiry_date', 'projects', 'id', $id);
				
				$expiry_date_new = date("Y-m-d",  strtotime('+30 day', strtotime($expiry_date)));
				$data['expiry_date'] = $expiry_date_new;
			}  */
			//parent::insert("projects", $data);
          ...
          $this->db->where('id', $id);
			...
			if ($this->db->update('projects', $data)){
				...
				if($feature_added){
					$expiry_date = getField('expiry_date', 'projects', 'id', $id);
					$expiry_date_new = date("Y-m-d",  strtotime('+30 day', strtotime($expiry_date)));
					$update_data['expiry_date'] = $expiry_date_new;
					$update_data['featured'] = $featured;
					$this->db->where('id', $id)->update('projects', $update_data);
				} 
				
				$freelancers = $this->input->post('freelancer');
				$p_name = getField('title','projects','project_id',$project_id);
				$username = getField('username','user','user_id',$user_id);
				$p_id_public = $project_id;
				if(!empty($freelancers) AND count($freelancers) > 0){
					foreach($freelancers as $v){
						
						$notification = 'You are invite for the project :'.$p_name;
						$link = 'job-'.seo_string($p_name).'-'.$p_id_public;
						$this->notification_model->log($user_id, $v, $notification, $link);
						...
						$mail_param = array(
							...
							'username' => $username,
							'project_name' => $p_name,
							'project_url' => base_url('job-'.seo_string($p_name).'-'.$p_id_public),
							'copy_url' => base_url('job-'.seo_string($p_name).'-'.$p_id_public),
						);
				...
				if($insufficient_bal){
					$payment_link = base_url('myfinance/add_fund_paypal?cmd=featured_project&project_id='.$project_id);
					$msg['message']="<h3 class='postjob-alert-title'><span style='margin-right:10px'><i class='fa fa-check'></i></span>Job Update Success.</h3>
					<p>You don't have sufficient fund in your wallet . Would you like to proceed to make this project featured ?</p> <ul class='alert-choice'><li><a  href='".$payment_link."'>Yes, Proceed </a></li><li><a href='".VPATH."jobdetails/details/".$project_id."'>No , Preview your job </a></li> <li><a  href='".VPATH."postjob/editjob/".$pid."'> Edit your job </a> </li></ul>";
	...
	public function edit_job_description($description='', $project_id=''){
		$user = $this->session->userdata('user');
		$data = array(
	...
	public function resetBid($project_id=''){
		$this->db->where(array('project_id' => $project_id))->update('bids', array('bidder_amt' => 0, 'total_amt' => 0));
		$this->db->where(array('project_id' => $project_id))->update('project_milestone', array('amount' => 0));
	}
