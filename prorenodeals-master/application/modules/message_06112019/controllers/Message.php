...
public function browse($project_id='', $other_user='') {
	...
	if(!$this->session->userdata('user')){
		...
		if(!empty($project_id) && !empty($other_user)){
			...
			$data['messages'] = $this->message_model->getAllMessageRight($project_id,$other_user,$user[0]->user_id);
...
public function get_left_msg($type=''){
	...
	$active_project = get('active_project');

	$project_user = get('project_user');
	...
	if(count($message) > 0){
		...
		foreach($message as $k => $v){
			...
			if($active_project && $active_project == $v['project_id'] && $v['sender_user'] == $project_user){
			...
			echo '<div class="setting-circle dropdown hidden"><i class="fa fa-ellipsis-h"></i></div><a id="anchor" href="javascript:void(0);" onclick="rampage(\''.$v['project_id'].'\',\''.$v['sender_user'].'\' , $(this));" data-project-id="'.$v['project_id'].'" data-project-user="'.$v['sender_user'].'" class="'.$active_class.'"><i class="far fa-comment"></i> <span class="menu active">'.$sender_fname.' , '.$recipient_fname .'<span class="pull-right hide" id="unread" style="color:red;">'.$v['unread_msg'].'</span></span><p>'.$project_name.'</p></a><p style="font-weight:normal;padding:5px 0px; display:none;">'.$msg.'</p></div>';
...
public function load_more_lft($limit=10){
	...
	if(count($message) > 0){
		...
		foreach($message as $k => $v){
			...
			echo '<div class="setting-circle dropdown hidden"><i class="fa fa-ellipsis-h"></i></div><a id="anchor" href="javascript:void(0);" onclick="rampage(\''.$v['project_id'].'\',\''.$v['sender_user'].'\' , $(this));" data-project-id="'.$v['project_id'].'" data-project-user="'.$v['sender_user'].'"><i class="far fa-comment"></i> <span class="menu active">'.$sender_fname.' , '.$recipient_fname .'<span class="pull-right hide" id="unread" style="color:red;">'.$v['unread_msg'].'</span></span><p>'.$project_name.'</p></a><p style="font-weight:normal;padding:5px 0px; display:none;">'.$msg.'</p></div>';
...
<?php if($val['sender_id']==$user_id){ ?><p class="chat_date <?php echo $val['read_status'] == 'Y' ? '' : 'sent_msg'; ?>"><?php echo $val['read_status'] == 'Y' ? 'Seen' : 'Sent'; ?></p><?php } ?>
