...
class Projectdashboard extends MX_Controller {
	...
	public function dispute_room($dispute_id=''){
		if(!$this->session->userdata('user')){
			redirect(VPATH."login/");	
		}  
		
		if(($dispute_id  > 0) == false){
			return false;
		}
		...
		$data['dispute_detail'] = $dispute_row;
		
		if(!$dispute_row || ($dispute_row['worker_id'] != $data['user_id'] && $dispute_row['employer_id'] != $data['user_id'])){
			return false;
		}
		
		
		$data['milestone_id'] = $dispute_row['milestone_id'];
		$data['project_id'] = $dispute_row['project_id'];
		
		$p_type = getField('project_type', 'projects', 'project_id', $dispute_row['project_id']);
		
		if($p_type == 'H'){
			
			$data['milestone_detail'] = $this->db->where('id', $dispute_row['milestone_id'])->get('milestone_payment')->row_array();
		
			$data['bid_detail'] = $this->db->where('id', $data['milestone_detail']['bid_id'])->get('bids')->row_array();
		
			$data['owner_id'] = $dispute_row['employer_id'];
			$data['freelancer_id'] = $dispute_row['worker_id'];
			
		}else{
			
			$data['milestone_detail'] = $this->db->where('id', $dispute_row['milestone_id'])->get('project_milestone')->row_array();
		...
		$breadcrumb=array(
			array(
					'title'=>'Dispute Room','path'=>''
			)
		);
		$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,'Dispute Room');
	...
	public function send_dispute_message(){
		...
		if($this->input->post()){
			...
			$track['dispute_id'] = $post['dispute_id'];			
			$track['milestone_id'] = $post['milestone_id'];			
			$track['project_id'] = $post['project_id'];			
			$track['last_respond_user'] = $post['sender_id'];			
			$track['last_respond_against_user'] = $post['receiver_id'];						
			$this->projectdashboard_model->update_time_track($track);
			...
			$json['last_respond_time'] = date('Y-m-d H:i:s');
			$json['status'] = 1;
			
			
			if($insert_id){
				$notification = "A new dispute message by ".$sender_info['fname'];
				$link = 'projectdashboard/dispute_room/'.$post['dispute_id'];
				$this->notification_model->log($post['sender_id'], $post['receiver_id'], $notification, $link);
				
				$this->_send_last_repond_msg($post['sender_id'], $post['receiver_id'], $post['project_id']);
			}
	...
	public function send_dispute_amount(){
		...
		if($this->input->post() && $user_id){
			...
			$responded_against_user = $send_to;						
			$track['dispute_id'] = $post['dispute_id'];
			$track['milestone_id'] = $post['milestone_id'];			
			$track['project_id'] = $post['project_id'];			
			$track['last_respond_user'] = $user_id;			
			$track['last_respond_against_user'] = $responded_against_user;						
			$this->projectdashboard_model->update_time_track($track);			
			...
			$this->_send_last_repond_msg($user_id, $send_to, $post['project_id']);
	...
	public function accept_dispute_request(){
		...
		if($h_id && $user_id){
			...
			if($dispute_requester != $user_id){
				...
				$dispute_id = $dispute_history_row['dispute_id'];
				...
				$track['dispute_id'] = $dispute_id;					
				$track['milestone_id'] = $milestone_id;					
				$track['project_id'] = $project_id;					
				$track['last_respond_user'] = $user_id;					
				$track['last_respond_against_user'] = $dispute_requester;					
				$track['dispute_status'] = 'S'; /* Settled */ 										
				$this->projectdashboard_model->update_time_track($track);		
	...
	public function deny_dispute_request(){
		...
		if($h_id && $user_id){
			...
			$dispute_id = getField('dispute_id', 'dispute_history', 'id', $h_id);	
			$track['dispute_id'] = $dispute_id;			
			$track['milestone_id'] = $milestone_id;			
			$track['project_id'] = $project_id;			
			$track['last_respond_user'] = $user_id;			
			$track['last_respond_against_user'] = $requested_by;						
			$this->projectdashboard_model->update_time_track($track);
			if($milestone_id && $project_id && $requested_by){
				...
				$this->_send_last_repond_msg($user_id, $requested_by, $project_id);
			}
	...
	public function send_to_admin(){
		...
		if($dispute_id > 0){
			...
			if($dispute_row['worker_id'] == $user_id || $dispute_row['employer_id'] == $user_id){
				$project_id = $dispute_row['project_id'];
				$milestone_id = $dispute_row['milestone_id'];
				
				$this->db->where('dispute_id', $dispute_id)->update('dispute', array('admin_involve' => 1));
			}else{
				$auth = false;
			}
		...
		if($project_id && $milestone_id && $auth){
		...
		if($auth){
			...
			if($user_id == $employer_id){			
				$responded_against_user = $freelancer_id;		
			}else{			
				$responded_against_user =$employer_id;		
			}
			$track['dispute_id'] = $dispute_id;		
			$track['milestone_id'] = $milestone_id;		
			$track['project_id'] = $project_id;		
	...
	private function _send_last_repond_msg($sender='', $receiver='', $project_id=''){
		
		$last_respond_time = date('d M,Y h:i A', strtotime("+".DISPUTE_RESPOND_TIME.' hours'));
		$project_title = getField('title', 'projects', 'project_id', $project_id);
		$fname = getField('fname', 'user', 'user_id', $sender);
		$to = getField('email', 'user', 'user_id', $receiver);
		$receiver_name = getField('fname', 'user', 'user_id', $receiver);
		$template = 'dispute-respond';
		$data_parse = array(
			'NAME' => $receiver_name,
			'USER' => $fname,
			'PROJECT' => $project_title,
			'LAST_RESPOND_TIME' => $last_respond_time,
		);
