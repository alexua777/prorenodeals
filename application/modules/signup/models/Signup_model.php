		...
		$account_type=$this->input->post('account_type');
 		$captcha=trim(strtolower($this->input->post('captcha'))); 
			...
			if( $account_type == 'F'){
				...
				$msg['errors'][$i]['message']=__('signup_please_enter_company_name','Please enter company name');					
				$i++;				
				}	
				if($phone==''){					
				$msg['status']='FAIL';					
				$msg['errors'][$i]['id']='phone';					
				$msg['errors'][$i]['message']=__('signup_please_enter_phone','Please enter contact phone number');					
				$i++;				
				}else if(!preg_match("/^[0-9]+$/", $phone)){
					$msg['status']='FAIL';					
					$msg['errors'][$i]['id']='phone';					
					$msg['errors'][$i]['message']= 'Please enter valid phone number';		
					$i++;	
				}					
			}else{
				$company='';
			...
			/* username checking */
			$this->load->model('api_login/api_login_model');
			$username = $this->api_login_model->generateUserName($fname);
			/* if($username=='' || strlen($username)<4 || strlen($username)>20 || !preg_match('/^[a-zA-Z0-9\._-]+$/',$username)){
				$msg['status']='FAIL';
				$msg['errors'][$i]['id']='regusername';
				$msg['errors'][$i]['message']= 'Username must be 4 to 20 characters, only letters , numbers , underscore, dash or dot allowed';
				$i++;
			}else{
...
//$hasNonAlphaNum = preg_match('b[\!\@#$%\?&\*\(\)_\-\+=]+bi', $password);
		...
		if($password=='' || !preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/", $password)){
			$msg['status']='FAIL';
			$msg['errors'][$i]['id']='regpassword';
			$msg['errors'][$i]['message']=__('signup_password_must_be_atleast_8','Password must be at least 8 characters, one uppercase letter, one lowercase letter, one number and one special character');
			$i++;
		...
		*/
		...
		if (!$this->my_validation->google_captcha_match()) {
		...
		if($i==0){
			...
			*/
			...
			$data_parse=array('username'=>strip_tags(trim($this->input->post('fname'))),
								'password'=>$password,
								'email'=>$email,
								'copy_url'=>$link,
								'url_link'=>$link
