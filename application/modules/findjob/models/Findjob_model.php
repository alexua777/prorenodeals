...
class Findjob_model extends BaseModel {
	...
	public function list_project($srch_param=array() , $limit=0 , $offset=40 , $for_list=TRUE){
		...
		$this->db->join("skills s", "s.id=project_skill.skill_id" , "LEFT");
		...
		if(!empty($srch_param['env']) AND $srch_param['env'] != 'All'){
			$this->db->where("$project.environment" , $this->db->escape_str($srch_param['env']));
		}
		
		if(!empty($srch_param['ptype']) AND $srch_param['ptype'] != 'All'){
			$this->db->where("$project.project_type" , $this->db->escape_str($srch_param['ptype']));
		}
		
		if(!empty($srch_param['featured']) AND $srch_param['featured'] != 'All'){
			$this->db->where("$project.featured" , $this->db->escape_str($srch_param['featured']));
		}
		...
		if(!empty($srch_param['q']) || !empty($srch_param['term'])){
			...
			$this->db->where("($project.title LIKE '%{$term}%' OR $project.description LIKE '%{$term}%' OR s.skill_name = '{$term}')");
		}
		...
		$this->db->group_by("$project.project_id");				if(!empty($srch_param['sort_by']) && in_array($srch_param['sort_by'], array('new', 'old'))){			if($srch_param['sort_by'] == 'new'){				$this->db->order_by("$project.id" , "DESC");			}else if($srch_param['sort_by'] == 'old'){				$this->db->order_by("$project.id" , "ASC");			}		}else{			$this->db->order_by("$project.featured" , 'ASC');			$this->db->order_by("$project.id" , "DESC");		}		
		if($for_list){
