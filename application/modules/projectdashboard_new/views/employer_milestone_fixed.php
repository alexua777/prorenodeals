              ...
              <div role="tabpanel" class="tab-pane active" id="overview">
                ...
                ?>
                    ...
                    ?></td>
                        ...
                        <td><a href="<?php echo base_url('/invoice/detail/'.$row['invoice_id'].'/'.'F')?>" target="_blank"><?php echo __('projectdashboard_milestone_invoice','Invoice'); ?></a> | <a href="javascript:void(0);" onclick="milestone_action('<?php echo VPATH;?>myfinance/releaseFund/<?php echo $row['id'];?>/A/?next=projectdashboard_new/employer/milestone/<?php echo $project_id; ?>', 'release')"><?php echo __('projectdashboard_milestone_relese','Release'); ?></a> 
...
function confirm_complete(){
	$.confirm({
		...
		buttons: {
			confirm: function () {
				mark_project_complete('<?php echo $project_id;?>');
			},
			cancel: function () {
...
<?php
$bidder_id = getField('bidder_id', 'projects', 'project_id', $project_id);
?>
function review_employer(){
	
	var url = '<?php echo base_url('review/load_ajax_page?page=rating_review&review_to_user='.$bidder_id.'&review_from_user='.$login_user_id.'&job_id='.$project_id.'&review_to=freelancer&next=projectdashboard_new/employer/overview/'.$project_id); ?>';
	load_ajax_modal(url);
