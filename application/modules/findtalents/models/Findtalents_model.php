...
class Findtalents_model extends BaseModel {
	...
	public function list_freelancer($srch_param=array() , $limit=0 , $offset=40 , $for_list=TRUE){
		...
		if(!empty($srch_param['skills'])){
			$skill_id_str = '';
			if(count($srch_param['skills']) > 0){
				$skill_id_str = $this->db->escape_str(implode(',', $srch_param['skills']));
			}
			$this->db->where("($user_skill.skill_id IN ($skill_id_str) OR $user_skill.sub_skill_id IN ($skill_id_str))");
		}
		if(!empty($srch_param['memplan']) AND $srch_param['memplan'] != 'All'){
			$this->db->where("$user.membership_plan" , $this->db->escape_str($srch_param['memplan']));
		}
		...
		if(!empty($srch_param['city']) AND $srch_param['city'] != 'All'){
			$this->db->where("$user.city" , $this->db->escape_str($srch_param['city']));
		}
		
		if(!empty($srch_param['q']) AND $srch_param['q'] != ''){
			$srch_param['q'] = $this->db->escape_like_str(trim($srch_param['q']));
			$this->db->where("((CONCAT_WS(' ',$user.fname,$user.lname) LIKE '%{$srch_param['q']}%') OR $user.company  LIKE '%{$srch_param['q']}%')");
		}
		
		if(!empty($srch_param['sort_by']) && in_array($srch_param['sort_by'], array('new', 'old'))){
			if($srch_param['sort_by'] == 'new'){
				$this->db->order_by("$user.featured", "DESC");
	...
	public function getUserSkills($user_id=''){
		...
		$this->db->select('ps.skill_name as parent_skill_name,s.skill_name as skill,s.skill_name,s.arabic_skill_name,s.spanish_skill_name,s.swedish_skill_name , ps.id as parent_skill_id , s.id as skill_id')
				...
				->join('skills s' , 's.id=us.sub_skill_id' , 'LEFT');
